 ...
 * history:  Added our handler based on axis CommonsHTTPSender so we can 
 * added headers to stop HTTP 1.1 chunking, add basic authentication, and
 * iControl sessions in v11+.
 */
public class IControlHTTPSender extends BasicHandler {
    
    /**
	 * 
	 */
	private static final long serialVersionUID = 8586798697822563185L;
	/** Field log           */
    protected static Log log = LogFactory.getLog(IControlHTTPSender.class.getName());    
    ...
    protected void initialize() {
        ...
        cm.getParams().setDefaultMaxConnectionsPerHost(4);
        cm.getParams().setMaxTotalConnections(40);
        cm.getParams().setConnectionTimeout(20);
        cm.getParams().setSoTimeout(5);
    ...
    public void invoke(MessageContext msgContext) throws AxisFault {
        ...
        if (log.isDebugEnabled()) {
            log.debug(Messages.getMessage("enter00","IControlHTTPSender::invoke"));
        }
        try {
        ...
        if (log.isDebugEnabled()) {
            log.debug(Messages.getMessage("exit00","iControlHTTPSender::invoke"));
        }
    }
	...
	private void addContextInfo(HttpMethodBase method, 
        ...
        if (userID != null) {
            	...
            	method.setRequestHeader("Authorization", "Basic " + authStringEnc);
    ...
    private static class MessageRequestEntity implements RequestEntity {
        ...
        boolean httpChunkStream = false; //Never use chunking.. EasySoap++ doesn't support it.
